// multi-mobile.js (CORREGIDO para botones en móvil)

// ... (todo el código hasta los eventos de usuario es igual) ...
let socket;
let estado = "1";
let mobileId = null;
const NUM_SIMULATED_MOBILES = 15;
let simulatedMobileIds = [];
let uiG;
...
function setup() {
  ...
  for (let i = 0; i < NUM_SIMULATED_MOBILES; i++) { simulatedMobileIds.push('sim-client-' + i + '-' + floor(random(1000, 9999))); }
  console.log(`Simulando ${simulatedMobileIds.length} clientes.`);
  socket = io();
  socket.on('connect', () => console.log('multi-mobile connected'));
...
function actualizarUI() { if (btnSI) { btnSI.remove(); btnSI = null; } if (btnNO) { btnNO.remove(); btnNO = null; } if (btnTap) { btnTap.remove(); btnTap = null; } if (estado === "1") { btnSI = createButton("SI"); styleButton(btnSI, "#b2ffb1ff"); btnSI.mousePressed(() => handleAnswerClick(true)); btnNO = createButton("NO"); styleButton(btnNO, "#ffb1b1ff"); btnNO.mousePressed(() => handleAnswerClick(false)); } else if (estado === "4") { btnTap = createButton("TAP"); styleButton(btnTap, "#d1d1d1ff"); btnTap.style('padding', '28px 28px'); btnTap.mousePressed(() => { console.log('Emitiendo 15 taps...'); for (const simId of simulatedMobileIds) { socket.emit('tap_mobile', true); } }); } actualizarUIPositions(); }
function handleAnswerClick(answer) { console.log(`Emitiendo 15 respuestas de '${answer}'...`); for (let i = 0; i < NUM_SIMULATED_MOBILES; i++) { socket.emit('answer_mobile', answer); } if (btnSI) { btnSI.attribute('disabled', '').style('background-color', '#ccc').style('filter', ''); } if (btnNO) { btnNO.attribute('disabled', '').style('background-color', '#ccc').style('filter', ''); } }
...
function touchStarted() {
  ...
  if (estado === "2" && !shattered) {
    // ... la lógica del shatter no cambia (emite 1 vez) ...
    ...
    const payload = { mobileId: mobileId || 'unknown-multi-mobile', frame: { dataURL, w: width, h: height }, shards: shards.map(s => ({ pos: {x:s.pos.x,y:s.pos.y,z:s.pos.z}, vel: {x:s.vel.x,y:s.vel.y,z:s.vel.z}, axis: {x:s.axis.x,y:s.axis.y,z:s.axis.z}, spin: s.spin, verts: s.verts.map(v => ({x:v.x,y:v.y})), uvs: s.uvs.map(u => ({x:u.x,y:u.y})) })) };
    socket.emit('phase_shatter', payload);
    return false;
  }

  if (estado === "3") {
    // ... la lógica de presionar no cambia (emite 15 veces) ...
    const { x, y, w, h } = pressBox;
    if (touchX >= x && touchX <= x + w && touchY >= y && touchY <= y + h) {
      isPressing = true;
      console.log('Emitiendo 15 destello_press (true)...');
      for (const simId of simulatedMobileIds) {
        socket.emit('destello_press', { mobileId: simId, pressing: true });
      }
    }
    return false;
  }
...
function touchEnded() {
  // CAMBIO: Añadimos un chequeo similar aquí.
  if (estado !== "3") {
    return;
  }
  
  if (isPressing) { // Ya sabemos que el estado es 3
    isPressing = false;
    console.log('Emitiendo 15 destello_press (false)...');
    for (const simId of simulatedMobileIds) {
      socket.emit('destello_press', { mobileId: simId, pressing: false });
    }
  }
  return false;
}
